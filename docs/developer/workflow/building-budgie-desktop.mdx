...
```bash
  sudo dnf install accountsservice-devel alsa-lib-devel budgie-screensaver budgie-session desktop-file-utils gettext git glib2-devel gnome-desktop3-devel gnome-settings-daemon-devel gobject-introspection-devel gsettings-desktop-schemas-devel gstreamer1-devel gtk-doc gtk3-devel ibus-devel intltool json-glib-devel libcanberra-devel libX11-devel libXtst-devel libgee-devel libgudev-devel libnotify-devel libpeas1-devel libuuid-devel libwacom-devel libwnck3-devel libxfce4windowing-devel meson magpie-devel polkit-devel pulseaudio-libs-devel sassc upower-devel vala
```
  </TabItem>
  <TabItem value="fedora-onyx" label="Fedora Onyx / Atomic">
For developing Budgie Desktop on top of Fedora Onyx / Atomic, we suggest rebasing on top of our [Atomic Dev](https://github.com/BuddiesOfBudgie/atomic-dev) ostree images. These are built daily on top of Onyx with the required development libraries, making additional package layer faster and less prone to breakage.
  </TabItem>
