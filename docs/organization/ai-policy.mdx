...
We take great pride in our work. Writing software is pragmatic by nature — it has to work, it has to be maintainable, and it has to be easy to reason about (at least, as easy as one can reason about an entire desktop environment). But we also treat writing code as a craft. We would rather move deliberately than move fast and break things. That care and intentionality is something we want to preserve in every contribution.
:::

## Our Stance

Budgie Desktop follows a **human-centric development** model. AI tools like Claude, Gemini, Copilot, ChatGPT, and others can be valuable — they can help explore ideas, draft implementations, accelerate development, and improve the clarity of communication everything from writing issues, pull requests, or blog posts.

But humans are the authors. Humans are accountable. AI is a tool in service of that goal, not a substitute for it.

Not all AI models and providers are the same — they vary in licensing, training data practices, openness, and how they relate to open source. For example, IBM's [Granite](https://github.com/ibm-granite) models are released under the Apache 2.0 license with disclosed training data sources and open-source data preparation tooling. This policy does not endorse or discourage any particular model or provider. Contributors are free to make their own informed choices about which tools they use.
...
All contributions must be tested before submission. When AI tools are involved, this is especially important — AI-generated code can be subtly wrong in ways that look correct at first glance.
...
"Trivial" is intentionally loosely defined. Typo fixes, documentation corrections, comment updates — these are trivial. If you're unsure whether your change qualifies, err on the side of testing it. When in doubt, build and install.
:::

### Attribution

When AI tools assisted with **code changes** (writing, modifying, or generating code), add an `Assisted-by` trailer to the commit message with the tool and model identifier:
...
- Ensure all relevant commits include the `Assisted-by` trailer
...
Content that is wholly generated by AI tooling without meaningful human involvement is never acceptable. Readers should never expect that a blog post or any form of written content by Buddies of Budgie was produced entirely by AI. There is always a human involved in the process — shaping the narrative, ensuring accuracy, and making it our own.

## This Policy Applies Equally

This policy is tool-agnostic. It applies to Claude, Gemini, Copilot, ChatGPT, locally-hosted models, and any other AI tool — current or future. The expectations are the same regardless of which tool you use.
