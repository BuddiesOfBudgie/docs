...
- eglinfo (provided by `egl-utils` on Fedora, `mesa-utils-extra` on Debian/Ubuntu, `mesa-demos` on Arch): This is used for querying EGL capabilities.
...
- dconf-cli
- eglinfo (provided by `egl-utils` on Fedora, `mesa-utils-extra` on Debian/Ubuntu, `mesa-demos` on Arch)
